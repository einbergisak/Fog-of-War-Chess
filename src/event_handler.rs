use ggez::{Context, GameResult, event::{EventHandler, MouseButton}, graphics::{self, Color, DrawMode, DrawParam, Image, Mesh, MeshBuilder, Rect, spritebatch::SpriteBatch}, nalgebra::Point2};

use crate::{Game, piece::{Piece, get_piece_rect}, render_utilities::{flip_board, flip_index}};
        ...
        match button {
            MouseButton::Left => {
                ...
                if let Some(piece) = self.board[index].take() {
                    ...
                    ggez::input::mouse::set_cursor_grabbed(ctx, true).expect("Cursor grabbed failed");
